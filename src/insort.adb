package body insort with SPARK_Mode => On is
   procedure more_than_sorted_after_insertion_point(A: Arr; sorted_to: Natural; value: Integer; insertion_point: Integer)
     with Ghost,
     Pre => (
            (A'First < Integer'Last and
                 sorted_to >= 0 and sorted_to >= A'First and sorted_to < (Integer'Last - 1) and sorted_to < A'Last)
     and then
       (insertion_point >= A'First and insertion_point < Integer'Last and insertion_point <= sorted_to + 1)
       and then
   ...
   function insert_point(A: Arr; sorted_to: Natural; value: Integer) return Natural
     with
       Pre => (
               (A'First < Integer'Last and
                 sorted_to > 0 and sorted_to > A'First and sorted_to < (Integer'Last - 1) and sorted_to < A'Last) and then
                 (
                  (sorted_to < Integer'Last) and
                    (A'First <= sorted_to + 1) and
                      (sorted_to < A'Last) and
                      (for all I in A'First + 1 .. sorted_to => A(I - 1) <= A(I))
                 )
              ),
       Post => (
                  insert_point'Result >= A'First and
                  insert_point'Result <= A'Last and
                  insert_point'Result < Integer'Last and
                  insert_point'Result <= sorted_to + 1 and
                    (for all I in A'First .. insert_point'Result - 1 => A(I) <= value) and
   ...
   procedure move_forward(A: in out Arr; from: Natural; to: Natural)
     with
       Pre => (
                 ((from < to) and
                   (A'First <= from) and
                   (to <= A'Last) and to <= Integer'Last-1 and to > 0)
               and then
   ...
   begin
      ...
      for I in from..to - 1 loop
         pragma Loop_Invariant((for all J in A'First .. from - 1 => A(J) <= v) and
                                 (for all J in from .. to - 1 => A(J) > v)
                                   and (for all J in A'First + 1 .. I => A(J - 1) <= A(J)));
         A(I + 1) := A(I);
      end loop;
   ...
   procedure swp(A: in out Arr; I, J: Natural)
     with
       Pre => A'First <= I and I < J and J <= A'Last,
       Post => ((A(I) = A'Old(J)) and (A(J) = A'Old(I)))
   is
