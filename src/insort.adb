...
is
   procedure more_than_sorted_after_insertion_point
     (A : Arr; sorted_to : Natural; value : Integer; insertion_point : Integer)
   with
     Ghost,
     Pre  =>
   ...
   function insert_point
     (A : Arr; sorted_to : Natural; value : Integer) return Natural
   with
     Pre  =>
       ((A'First < Integer'Last
         and then sorted_to > 0
   ...
   with
     Pre  =>
       (((from < to)
         ...
         and then to > 0)
        and then ((for all I in A'First + 1 .. to - 1 => A (I - 1) <= A (I))
                  and then (for all I in from .. to - 1 => A (to) < A (I))
                  and then (for all I in A'First .. from - 1
                            => A (I) <= A (to)))),
     Post =>
       (A (from) = A'Old (to)
        and then (for all I in A'First + 1 .. to => A (I - 1) <= A (I)))
   is
   ...
   begin
      ...
      for I in reverse from .. to - 1 loop
         pragma
           Loop_Invariant
             ((for all J in A'First .. from - 1 => A (J) <= v)
                and then (for all J in from .. to - 1 => A (J) > v)
                and then (for all J in A'First + 1 .. to
                          => A (J - 1) <= A (J)));
         A (I + 1) := A (I);
      end loop;
   ...
   begin
      ...
      for I in A'First + 1 .. A'Last - 1 loop
         pragma
           Loop_Invariant
             (for all J in A'First + 1 .. I => A (J - 1) <= A (J));
         --  pragma Assert (I > 0);
